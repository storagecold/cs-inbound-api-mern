...
test('Empty bankAccountName', () => {
    account.bankAccountName = '';
    const {error} = accountUtils.accountValidate(account);
    expect(error).not.toBeUndefined();
    expect(error.details[0].message).toContain('\"bankAccountName\" is not allowed');
});

test('Non-string value as the bankAccountName', () => {
