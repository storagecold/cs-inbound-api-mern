...
test('Empty IFSC code', () => {
    account.ifscCode = '';
    const {error} = accountUtils.accountValidate(account);
    expect(error).not.toBeUndefined();
    expect(error.details[0].message).toContain('\"ifscCode\" is not allowed');

});


test('IFSC code with invalid length', () => {
    account.ifscCode = 'ABCD12345';
    const {error} = accountUtils.accountValidate(account);
    expect(error).not.toBeUndefined();
    expect(error.details[0].message).toContain('\"ifscCode\" is not allowed');
});

test('Non-string value as the IFSC code', () => {
