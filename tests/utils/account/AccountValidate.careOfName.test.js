...
test('Valid care of name', () => {
    account.careOfName;
    const { error,value } = accountUtils.accountValidate(account);
    expect(error).toBeUndefined();
    expect(value.careOfName).toContain(account.careOfName);
});

test('Empty care of name', () => {
    account.careOfName = '';
    const { error } = accountUtils.accountValidate(account);
    expect(error).not.toBeUndefined();
    expect(error.details[0].message).toContain('\"careOfName\" is not allowed to be empty');
});

test('Non-string value as the care of name', () => {
     account.careOfName = null;
    const { error } = accountUtils.accountValidate(account);
    expect(error).not.toBeUndefined();
    expect(error.details[0].message).toContain('\"careOfName\" must be a string');
});
