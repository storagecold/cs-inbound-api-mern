...
test('Valid last name', () => {
    ...
    expect(value.lastName).toContain(account.lastName);
});
...
test('Empty lastName', () => {
    ...
    expect(error.details[0].message).toContain('\"lastName\" is not allowed to be empty');
});

test('Non-string value as the lastName', () => {
    account.lastName = null;
    const {error} = accountUtils.accountValidate(account);
    expect(error).not.toBeUndefined();
    expect(error.details[0].message).toContain('\"lastName\" must be a string');
});
