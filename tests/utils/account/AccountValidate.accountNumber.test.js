...
test('Valid accountNumber', () => {
    const {error, value} = accountUtils.accountValidate(account);
    expect(error).toBeUndefined();
    expect(value.accountNumber).toEqual(account.accountNumber);
});

test('Empty accountNumber', () => {
    account.accountNumber = '';
    const {error} = accountUtils.accountValidate(account);
    expect(error).not.toBeUndefined();
    expect(error.details[0].message).toContain('\"accountNumber\" must be a number');
});
