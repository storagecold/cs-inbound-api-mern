...
test('should pass validation for a valid organization name', () => {
...
test('should fail validation when organization is missing', () => {
    ...
    expect(error.details[0].message).toContain('"organization" is required');
});

test('should fail validation when organization is an empty string', () => {
     company.organization = " ";
    const { error } = companyUtils.comapnyValidate(company);
    expect(error).not.toBeUndefined();
    expect(error.details[0].message).toContain('\"organization\" is not allowed to be empty');
});
